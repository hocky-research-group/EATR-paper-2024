...
def get_data(directory):
    ...
    plogs = []
    for run in runs:
        colvars.append(f"../../Data/E_end_end_distance_opes/{directory}/{run}/{colvar_name}")
        plogs.append(f"../../Data/E_end_end_distance_opes/{directory}/{run}/{log_name}")
    ...
    for colvar in colvars:
        current_colvar = np.loadtxt(colvar, usecols=[0,4])
...
for barr in barrs:
    trans_times = np.loadtxt(f"../../Data/E_end_end_distance_opes/eruns_barr{barr}/trans_times.dat", usecols=0)
    emp_rates.append(1. / np.mean(trans_times))
    data, final_times, colvars_count, colvars_maxrow_count, event = get_data(f"eruns_barr{barr}")
